...
<div id="toPage" class="linkBtn"><a href="./">管理画面トップへ</a></div>
<h1>編集画面</h1>
<p>投稿記事を編集します。編集後、ページ下部の「登録」ボタンを押して下さい。（ボタンを押すまで反映されません）</p>
...
<?php
	...
	foreach($extensionList as $val){
		...
		if(file_exists($upFilePath)){
			...
			echo '　<input type="checkbox" name="link_file_del['.str_replace($img_updir.'/','',$upFilePath).']" value="true"> 削除';
			break;
		}
	}
?>
...
for($i=0;$i<=$maxCommentCount;$i++){
	...
	foreach($extensionList as $val){
		...
		if(file_exists($upFilePath)){
			...
			$upfileTag .= '<br /><span id="upfile_del'.($i + 1).'"><input type="checkbox" name="upfile_del[]" value="'.str_replace($img_updir.'/','',$upFilePath).'" /></span> 削除';
			break;
		}
	}
...
<tr class="lines<?php echo $i + 1;?>">
<th>ファイル（5MB以内）<?php echo $upfileTag;?><input type="hidden" name="upfile_name[]" value="<?php echo (!empty($upfileTag)) ? str_replace($img_updir.'/','',$upFilePath) : '';?>" /></th>
<td><input type="file" name="data[upfile][]" /></td>
</tr>
<tr class="lines<?php echo $i + 1;?>">
<td colspan="3" style="border:0;height:10px;padding:0"></td>
