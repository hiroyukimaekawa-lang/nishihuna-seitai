	...
	if($mode == 'update' && isset($_POST["upfile_del"])){
		foreach($_POST["upfile_del"] as $key => $val){
			if(!empty($val)){
				...
				if(strpos($val,'/') !== false || strpos($val,'..') !== false) exit('FILEPATH ERROR');//ディレクトリトラバーサル対策
				
				$imgDelPathS = $img_updir.'/'.str_replace('.','s.',$val);//サムネイル削除
				$imgDelPathL = $img_updir.'/'.$val;
	...
	if($mode == 'update' && isset($_POST["link_file_del"])){
		foreach($_POST["link_file_del"] as $key => $val){
			if($val == 'true' && !empty($key)){
				if(strpos($key,'/') !== false || strpos($key,'..') !== false) exit('FILEPATH ERROR');//ディレクトリトラバーサル対策
				$res = (file_exists($img_updir.'/'.$key)) ? unlink($img_updir.'/'.$key) : '';
			}
		}
	}
		...
		foreach($_POST['upfile_name'] as $key => $val){
			if(!empty($val)){
				...
				if($fileNum != $key){
					...
					rename($img_updir.'/'.$val,$newName);//リネーム
